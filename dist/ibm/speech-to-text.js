  ...
  function micON() { // When the microphone button is clicked
    if (recording === false) {
      if (records === 0) { // The first time the mic is clicked - inform user
        Api.setWatsonPayload({output: {text: ['Acepta la utilización del microfono en tu navegador. Watson te escuchará inmediatamente.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
        records++;
      } else {
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              Api.setWatsonPayload({output: {text: ['Micrófono deshabilitado. Por favor presiona el botón para hablar a Watson.']}}); // If the user clicked the microphone button again to cancel current input
            }
          })
          .catch(function(err) { // Catch any errors made during the promise
            if (err !== 'Error: Ninguna voz escuchada en los últimos 5s.') { // This error will always occur when Speech-To-Text times out, so don't log it (but log everything else)
              console.log(err);
            }
            mic.setAttribute('class', 'inactive-mic'); // Reset our microphone button to visually indicate we aren't listening to user anymore
            Api.setWatsonPayload({output: {text: ['Watson termina después de algunos segundos de inactividad. Por favor presiona el botón para hablar a Watson.']}});
          });
      })
